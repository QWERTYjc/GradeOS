...
import { DeleteOutlined, EyeOutlined, ArrowLeftOutlined, ArrowRightOutlined, CheckOutlined } from '@ant-design/icons';
      ...
      <div className="flex-1 overflow-auto p-3">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {images.map((img, index) => (
            <div
              key={img.id}
              className={`relative group bg-white rounded-lg shadow-sm border overflow-hidden transition-all ${
                selectedIds.has(img.id) ? 'ring-2 ring-blue-500' : ''
              }`}
            >
              <div className="aspect-[3/4] relative bg-gray-100">
                ...
                >
                  {selectedIds.has(img.id) && <CheckOutlined className="text-xs" />}
                </button>
