...
<cite>
**本文档中引用的文件**
- [result_formatter.py](file://ai_correction/functions/langgraph/result_formatter.py) - *更新了详细报告生成逻辑*
- [result_aggregator_agent.py](file://ai_correction/functions/langgraph/agents/result_aggregator_agent.py) - *增强了结果聚合功能*
- [class_analysis_agent.py](file://ai_correction/functions/langgraph/agents/class_analysis_agent.py) - *改进了班级分析算法*
...
## 更新摘要
**主要变更**
- 全面更新了`format_grading_result_v2`的渲染逻辑，增强逐题分析的可视化效果
- 详细说明了`_format_single_question_result`与`_format_scoring_detail`的嵌套调用关系
- 扩展了`ResultAggregatorAgent`的学生报告生成机制
- 深入解析了`ClassAnalysisAgent`的班级整体分析算法
- 新增了报告模板自定义和样式修改指南
...
`_format_single_question_result`函数负责格式化单个题目的批改结果，其主要处理流程包括：
- 题目标题展示（截取前100字符）
- 总体成绩计算与显示
- 学生答案展示（限制500字符，过长则截断）
- 逐点评分详情处理
- 总体反馈、优点、不足和改进建议的展示

`_format_scoring_detail`函数则专门处理单个评分点的详情，其输出包含：
- 评分点状态图标（✅表示正确，❌表示错误）
- 评分点名称、ID和分值
- 得分情况
- 分析、证据和原因说明
...
`ResultAggregatorAgent`通过`_generate_student_report`方法为每个学生生成详细报告，该方法调用多个辅助方法：
- `_calculate_grade_level`：根据总分和满分计算等级（A-F）
- `_generate_feedback`：生成评分准则的反馈文本
- `_extract_strengths`：提取满足的评分准则作为优点
- `_extract_improvements`：提取未满足的评分准则作为改进点
...
`ClassAnalysisAgent`在执行时首先检查`enable_class_analysis`状态标志，然后验证学生数量是否达到2人以上。其核心分析功能包括：
- `_calculate_distribution`：计算各等级（A-F）的学生分布
- `_identify_common_issues`：识别失分最多的评分点，按失败次数排序并计算失败率
