# 生产级 LangGraph AI 批改系统 - 实施计划

## 0. LangGraph 8 Agent 升级快照
- **依赖设计**：本计划与 `langgraph_correction_system_design.md` 的 8 Agent 拓扑保持一致，所有任务需同步更新 `PROMPT_CHANGELOG.md`。
- **周度节奏**：
  1. 第 1 周聚焦 Intake/Context/Rubric 三节点与测试夹具。
  2. 第 2 周实现 Evaluation Coordinator + Question Grader Pool，并完成并行压测。
  3. 第 3 周补齐 Feedback/QA/Report，打通数据库/前端 Demo。
  4. 第 4 周完成性能/容错压测与上线评审材料。
- **验收门槛**：每周结束前需输出周报模板（状态/风险/指标），并由 LangGraph Owner 及 Prompt Reviewer 双签确认，确保“设计-实现-监控”闭环。

## 📅 开发阶段规划

### Phase 1: 核心功能实现（第1-2周）
**目标**：实现基础的逐题批改功能

#### 1.1 InputParser Agent 实现
- **工作量**：2天
- **任务**：
  - [ ] 实现题目解析逻辑（正则表达式识别题目边界）
  - [ ] 实现学生信息提取（从文件名和文本）
  - [ ] 实现答案匹配（将答案与题目关联）
  - [ ] 编写单元测试

**技术难点**：
- 题目边界识别（不同格式的题目编号）
- 学生信息提取（多种格式支持）

**解决方案**：
- 使用正则表达式库处理多种格式
- 支持用户自定义题目分隔符

#### 1.2 QuestionAnalyzer Agent 实现
- **工作量**：2天
- **任务**：
  - [ ] 实现题型识别（选择题、填空题、解答题等）
  - [ ] 实现难度评估
  - [ ] 实现批改策略选择
  - [ ] 编写单元测试

**技术难点**：
- 准确的题型识别
- 难度评估的准确性

**解决方案**：
- 使用LLM辅助题型识别
- 基于题目长度和关键词的难度评估

#### 1.3 RubricInterpreter Agent 实现
- **工作量**：2天
- **任务**：
  - [ ] 实现评分标准解析
  - [ ] 实现评分维度提取
  - [ ] 实现评分规则生成
  - [ ] 编写单元测试

**技术难点**：
- 从自由格式的评分标准中提取结构化信息

**解决方案**：
- 使用LLM进行智能解析
- 提供标准化的评分标准模板

#### 1.4 QuestionGrader Agent 实现
- **工作量**：3天
- **任务**：
  - [ ] 实现基础批改逻辑
  - [ ] 实现错误识别
  - [ ] 实现反馈生成
  - [ ] 实现知识点提取
  - [ ] 编写单元测试

**技术难点**：
- 准确的错误识别
- 有意义的反馈生成
- 知识点的准确提取

**解决方案**：
- 使用精心设计的prompt
- 多轮LLM调用确保质量
- 建立知识点库

#### 1.5 ResultAggregator Agent 实现
- **工作量**：1天
- **任务**：
  - [ ] 实现结果聚合逻辑
  - [ ] 实现统计计算
  - [ ] 实现报告生成
  - [ ] 编写单元测试

#### 1.6 LangGraph 工作流集成
- **工作量**：2天
- **任务**：
  - [ ] 设计工作流图
  - [ ] 实现Agent节点
  - [ ] 实现边和路由
  - [ ] 实现流式处理
  - [ ] 编写集成测试

**技术难点**：
- 流式处理的实现
- 并行处理的协调

**解决方案**：
- 使用LangGraph的stream API
- 使用条件边实现动态路由

---

### Phase 2: 数据库和持久化（第3周）
**目标**：实现数据存储和查询功能

#### 2.1 数据库设计和创建
- **工作量**：1天
- **任务**：
  - [ ] 设计表结构
  - [ ] 创建数据库
  - [ ] 创建索引
  - [ ] 编写迁移脚本

#### 2.2 DataPersistence Agent 实现
- **工作量**：2天
- **任务**：
  - [ ] 实现数据存储逻辑
  - [ ] 实现关系管理
  - [ ] 实现事务处理
  - [ ] 编写单元测试

#### 2.3 数据查询接口
- **工作量**：2天
- **任务**：
  - [ ] 实现学生查询
  - [ ] 实现批改结果查询
  - [ ] 实现统计数据查询
  - [ ] 编写API文档

---

### Phase 3: Streamlit 集成（第4周）
**目标**：实现用户界面和交互

#### 3.1 UI 设计和实现
- **工作量**：3天
- **任务**：
  - [ ] 设计UI流程
  - [ ] 实现文件上传
  - [ ] 实现学生信息输入
  - [ ] 实现批改模式选择
  - [ ] 实现进度显示
  - [ ] 实现结果展示

**技术难点**：
- 实时进度更新
- 大文件上传处理

**解决方案**：
- 使用Streamlit的session_state
- 使用分块上传

#### 3.2 数据分析和可视化
- **工作量**：2天
- **任务**：
  - [ ] 实现统计数据展示
  - [ ] 实现图表可视化
  - [ ] 实现数据导出

#### 3.3 集成测试
- **工作量**：2天
- **任务**：
  - [ ] 端到端测试
  - [ ] 性能测试
  - [ ] 用户体验测试

---

### Phase 4: 优化和部署（第5周）
**目标**：性能优化和生产部署

#### 4.1 性能优化
- **工作量**：2天
- **任务**：
  - [ ] Token使用优化
  - [ ] 缓存机制实现
  - [ ] 并行处理优化
  - [ ] 性能测试和基准

#### 4.2 错误处理和日志
- **工作量**：1天
- **任务**：
  - [ ] 实现错误处理
  - [ ] 实现日志记录
  - [ ] 实现监控告警

#### 4.3 部署和上线
- **工作量**：2天
- **任务**：
  - [ ] 部署到Railway
  - [ ] 配置环境变量
  - [ ] 进行上线测试
  - [ ] 编写部署文档

#### 4.4 文档和培训
- **工作量**：1天
- **任务**：
  - [ ] 编写用户手册
  - [ ] 编写API文档
  - [ ] 编写运维手册
  - [ ] 进行用户培训

---

## 🎯 优先级排序

### 必做项（P0）
1. **InputParser Agent** - 基础功能
2. **QuestionGrader Agent** - 核心功能
3. **ResultAggregator Agent** - 结果汇总
4. **LangGraph 工作流** - 流程编排
5. **DataPersistence Agent** - 数据存储
6. **Streamlit 基础UI** - 用户界面

### 重要项（P1）
1. **QuestionAnalyzer Agent** - 题型识别
2. **RubricInterpreter Agent** - 评分标准解析
3. **流式处理** - 实时反馈
4. **数据分析** - 统计功能
5. **缓存机制** - 性能优化

### 可选项（P2）
1. **高级可视化** - 数据展示
2. **导出功能** - 结果导出
3. **多语言支持** - 国际化
4. **断点续传** - 容错能力

---

## 🔧 技术难点和解决方案

### 难点 1：准确的题型识别
**问题**：不同格式的题目，题型识别困难

**解决方案**：
- 使用LLM辅助识别
- 建立题型特征库
- 支持用户手动指定

**预估工作量**：2天

---

### 难点 2：有意义的反馈生成
**问题**：生成的反馈过于笼统或不准确

**解决方案**：
- 精心设计prompt
- 多轮LLM调用
- 建立反馈模板库
- 人工审核和优化

**预估工作量**：3天

---

### 难点 3：知识点的准确提取
**问题**：从答卷中准确提取知识点困难

**解决方案**：
- 建立知识点库
- 使用LLM进行语义匹配
- 支持教师自定义知识点

**预估工作量**：2天

---

### 难点 4：流式处理的实现
**问题**：实现实时的流式处理和进度显示

**解决方案**：
- 使用LangGraph的stream API
- 使用Streamlit的session_state
- 使用WebSocket或Server-Sent Events

**预估工作量**：2天

---

### 难点 5：Token成本控制
**问题**：大量批改导致Token成本过高

**解决方案**：
- 压缩输入内容
- 实现缓存机制
- 使用更便宜的模型
- 批量处理优化

**预估工作量**：2天

---

## 📊 工作量估计

| 阶段 | 任务 | 工作量 | 人数 |
|------|------|--------|------|
| Phase 1 | 核心Agent实现 | 12天 | 2人 |
| Phase 2 | 数据库和持久化 | 5天 | 1人 |
| Phase 3 | Streamlit集成 | 7天 | 1人 |
| Phase 4 | 优化和部署 | 6天 | 1人 |
| **总计** | | **30天** | **2人** |

**预估时间**：4-5周（2人团队）

---

## ✅ 验收标准

### Phase 1 验收
- [ ] 所有Agent单元测试通过
- [ ] 工作流集成测试通过
- [ ] 能处理至少10道题目的批改
- [ ] 批改结果准确度 > 80%

### Phase 2 验收
- [ ] 数据库表创建成功
- [ ] 数据存储和查询功能正常
- [ ] 数据一致性检查通过

### Phase 3 验收
- [ ] UI功能完整
- [ ] 用户交互流畅
- [ ] 进度显示准确
- [ ] 结果展示清晰

### Phase 4 验收
- [ ] 性能指标达到目标
- [ ] 错误处理完善
- [ ] 部署成功
- [ ] 文档完整

---

## 🚀 快速启动指南

### 开发环境设置
```bash
# 1. 克隆仓库
git clone https://github.com/ZkwareDAO/aiguru.git
cd aiguru

# 2. 创建虚拟环境
python -m venv .venv
source .venv/bin/activate  # Windows: .venv\Scripts\activate

# 3. 安装依赖
pip install -r requirements.txt
pip install langgraph langchain

# 4. 配置环境变量
cp .env.example .env
# 编辑 .env，填入 API 密钥等

# 5. 初始化数据库
python scripts/init_db.py

# 6. 启动开发服务器
streamlit run ai_correction/streamlit_simple.py
```

### 测试
```bash
# 运行单元测试
pytest tests/

# 运行集成测试
pytest tests/integration/

# 性能测试
python tests/performance/benchmark.py
```

---

## 📞 联系和支持

- **技术负责人**：[待定]
- **项目经理**：[待定]
- **文档**：[待定]
- **问题反馈**：[待定]

