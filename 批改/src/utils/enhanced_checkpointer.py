            ...
            async with self.pool_manager.pg_connection() as conn:
                ...
                if row["is_delta"] and row["base_checkpoint_id"]:
                    checkpoint = await self._rebuild_from_delta(
                        ...
                        row["base_checkpoint_id"], checkpoint_data
                    )
                else:
    ...
    async def _rebuild_from_delta(
        ...
        if row["is_delta"] and row["base_checkpoint_id"]:
            base_state = await self._rebuild_from_delta(
                conn, thread_id, checkpoint_ns,
                row["base_checkpoint_id"], base_data
            )
        else:
    ...
    async def aput(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                last_error = e
                logger.warning(
                    f"检查点保存失败 (尝试 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt < self.max_retries - 1:
                    await asyncio.sleep(self.RETRY_DELAY * (attempt + 1))
                    self._report_heartbeat(
                        "checkpoint_put_retry", 
        ...
        async with self.pool_manager.pg_transaction() as conn:
            ...
            if parent_checkpoint_id:
                ...
                result = await conn.execute(
                    query, (thread_id, checkpoint_ns, parent_checkpoint_id)
                )
                row = await result.fetchone()
                ...
                if row:
                    ...
                    if row["is_delta"] and row["base_checkpoint_id"]:
                        previous_checkpoint = await self._rebuild_from_delta(
                            conn, thread_id, checkpoint_ns,
                            row["base_checkpoint_id"], prev_data
                        )
                        # 使用原始基础检查点
            ...
            if previous_checkpoint:
                ...
                if is_delta and delta:
                    ...
                    data_to_save = data_to_save.encode("utf-8") if isinstance(data_to_save, str) else data_to_save
            else:
                # 没有前一个检查点，保存完整状态
                _, data_to_save = self.serde.dumps_typed(checkpoint)
                data_to_save = data_to_save.encode("utf-8") if isinstance(data_to_save, str) else data_to_save
    ...
    async def aput_writes(
        ...
        async with self.pool_manager.pg_transaction() as conn:
            for idx, (channel, value) in enumerate(writes):
                ...
                await conn.execute(
                    insert_query,
                    (thread_id, checkpoint_ns, checkpoint_id, task_id, idx, channel, type_str, blob_bytes)
                )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    await asyncio.sleep(self.RETRY_DELAY * (attempt + 1))
                    self._report_heartbeat(
                        "checkpoint_put_with_grading_retry", 
                        (attempt + 1) / self.max_retries
                    )
