...
class RegressionTester:
    ...
    def __init__(
        ...
        max_degradation_rate: float = 0.02
    ):
        """初始化回归测试器
    ...
    async def run_regression(
        ...
        # 运行旧版本（不应用补丁）
        old_results = await self._run_grading_batch(
            eval_samples,
            apply_patch=False,
            patch=None
        )
        ...
        # 运行新版本（应用补丁）
        new_results = await self._run_grading_batch(
            eval_samples,
            apply_patch=True,
            patch=patch
        )
    ...
    async def _run_grading_batch(
        ...
        # 这里返回模拟数据用于测试
        logger.info(
            f"运行批改：{'应用补丁' if apply_patch else '不应用补丁'}，"
            f"样本数 {len(samples)}"
        )
