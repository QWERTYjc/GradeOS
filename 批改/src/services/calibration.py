...
class CalibrationService:
    ...
    async def _get_profile_by_teacher_id(
        self,
        teacher_id: str
        ...
        try:
            async with self.pool_manager.pg_connection() as conn:
                ...
                # 解析 JSONB 字段
                tolerance_rules = [
                    ToleranceRule(**rule) for rule in row['tolerance_rules']
                ]
        ...
        try:
            ...
            for rule in profile.tolerance_rules:
                if rule.rule_type == "numeric":
                    ...
                    # 同义词容差
                    if self._check_synonym_tolerance(
                        student_answer, standard_answer
                    ):
                        return True
    ...
    def generate_feedback(
        ...
        try:
            ...
            if not template:
                # 使用默认模板
                template = DEFAULT_FEEDBACK_TEMPLATES.get(
                    scenario,
                    "评分完成"
                )
