...
class RubricService:
    ...
    async def create_rubric(
        ...
        if exists:
            raise ValueError(
                f"评分细则已存在: exam_id={request.exam_id}, "
    ...
    async def get_rubric(
        ...
        验证：需求 9.2
        """
        rubric_data = await self.repository.get_by_exam_and_question(
            exam_id,
            question_id
        )
        ...
        if rubric_data is None:
            logger.debug(
                f"评分细则不存在: exam_id={exam_id}, question_id={question_id}"
            )
            return None
    ...
    async def update_rubric(
        ...
        deleted_count = await self.cache_service.invalidate_by_rubric(
            old_rubric_text
        )
        ...
        logger.info(
            f"更新评分细则成功: rubric_id={rubric_id}, "
            f"使 {deleted_count} 条缓存失效"
        )
        
        # 如果评分细则文本也更新了，还需要使新文本的缓存失效并重新预计算哈希
        if request.rubric_text is not None and request.rubric_text != old_rubric_text:
            new_deleted_count = await self.cache_service.invalidate_by_rubric(
                request.rubric_text
            )
            logger.info(
                f"新评分细则文本缓存失效: {new_deleted_count} 条"
            )
    ...
    async def delete_rubric(self, rubric_id: str) -> bool:
        ...
        if success:
            logger.info(
                f"删除评分细则成功: rubric_id={rubric_id}, "
                f"使 {deleted_count} 条缓存失效"
            )
        
        return success
    
    async def check_rubric_exists(
        self,
        exam_id: str,
        question_id: str
    ) -> bool:
    ...
    async def validate_rubric_for_grading(
        ...
        if not exists:
            error_msg = (
                f"题目缺失评分细则，需要手动配置: "
                f"exam_id={exam_id}, question_id={question_id}"
            )
