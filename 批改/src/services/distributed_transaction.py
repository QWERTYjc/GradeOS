...
class DistributedTransactionCoordinator:
        ...
        try:
            ...
            for step in steps:
                ...
                try:
                    ...
                    self._logger.debug(
                        f"Saga {saga_id}: 步骤 '{step.name}' 完成"
                    )
                    ...
                    self._logger.error(
                        f"Saga {saga_id}: 步骤 '{step.name}' 失败: {e}"
                    )
                    
                    # 触发补偿
                    compensation_success = await self.compensate(
                        saga_id, completed_steps
                    )
            ...
            if self.enable_logging:
                await self.log_transaction(
                    saga_id, steps, final_status, started_at
                )
    ...
    async def compensate(
        ...
        self._logger.info(
            f"Saga {saga_id}: 开始补偿操作，需补偿 {len(completed_steps)} 个步骤"
        )
        ...
        for step in reversed(completed_steps):
            ...
            for attempt in range(self.max_compensation_retries):
                try:
                    ...
                    self._logger.debug(
                        f"Saga {saga_id}: 步骤 '{step.name}' 补偿成功"
                    )
                    break
            ...
            if not success:
                ...
                self._logger.error(
                    f"Saga {saga_id}: 步骤 '{step.name}' 补偿最终失败，需要人工干预"
                )
        ...
        if all_compensated:
            ...
            self._logger.error(
                f"Saga {saga_id}: 部分补偿操作失败，需要人工干预"
            )
    ...
    async def list_failed_transactions(
        self,
        limit: int = 100
    ) -> List[SagaTransaction]:
        """
        列出需要人工干预的失败事务
    ...
    async def cleanup_partial_state(
        self,
        saga_id: str,
        cleanup_actions: Optional[List[Callable[[], Awaitable[None]]]] = None
    ) -> bool:
        """
        清理部分写入状态
        ...
        if cleanup_actions:
            for i, action in enumerate(cleanup_actions):
                try:
                    await action()
                    self._logger.debug(
                        f"Saga {saga_id}: 清理操作 {i + 1} 成功"
                    )
                except Exception as e:
                    all_cleaned = False
                    self._logger.error(
                        f"Saga {saga_id}: 清理操作 {i + 1} 失败: {e}"
                    )
