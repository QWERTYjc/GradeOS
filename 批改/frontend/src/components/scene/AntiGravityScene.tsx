...
// New Component: Floating Circles
const FloatingCircles = () => {
    // Opacities: 3%, 6%, 10%, 10%, 15%, 4%
    // Sizes: Varying sizes
    // Speed adjustments:
    // First 3: Reduced by 30% (x0.7)
    // Last 3: Reduced by 70% (x0.3)
    ...
    // Baby Blue Color: #AEC9D6 -> RGB(0.682, 0.788, 0.839)
        ...
        <>
            {config.map((c, i) => (
                <mesh key={i} ref={el => meshRefs.current[i] = el}>
                    {/* Use PlaneGeometry for shader-based circle drawing */}
...
const CameraController = () => {
    ...
    useFrame((state) => {
        // Gentle float for the camera, much calmer than before
