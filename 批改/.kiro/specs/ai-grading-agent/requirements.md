...
本文档定义了一套生产级、纯视觉（Vision-Native）AI 批改系统的需求规范。该系统摒弃传统 OCR 方案，直接利用多模态大模型（Gemini 3.0 Pro 和 Gemini 2.5 Flash Lite）对试卷图像进行端到端的语义理解与评估。系统采用 Temporal 作为分布式工作流编排引擎，结合 LangGraph 构建题内智能体，实现对学生解题步骤的深度逻辑验证。
...
**用户故事：** 作为批改系统，我希望使用 LangGraph 智能体对每道题目进行深度推理批改，以便评分结果准确且可解释。

#### 验收标准

1. 当批改智能体接收到题目图像和评分细则时，智能体应当执行包含视觉提取、评分映射和自我反思的多步推理过程
2. 当视觉提取节点处理图像时，节点应当生成学生解题步骤的详细文字描述
3. 当评分映射节点评估解答时，节点应当将每个评分点映射到视觉分析中的具体证据
4. 当反思节点审查评分时，节点应当识别潜在的评分错误并生成修正反馈
5. 当存在反思反馈且修正次数少于 3 次时，智能体应当带着反馈循环回到评分节点
6. 当智能体完成推理时，智能体应当输出最终分数、置信度分数和推理轨迹
7. 当每次状态转换发生时，LangGraph 运行时应当将检查点持久化到 PostgreSQL 以供审计追踪
...
**用户故事：** 作为教师，我希望查看 AI 批改的完整推理过程，以便我能够理解评分依据并进行复核。

#### 验收标准

1. 当题目批改完成时，存储服务应当将分数、置信度、视觉标注和智能体轨迹持久化到 PostgreSQL
2. 当存储视觉标注时，服务应当包含用于在原图上高亮错误的边界框坐标
3. 当存储智能体轨迹时，服务应当以 JSONB 格式包含视觉分析、推理步骤和反思反馈
4. 当用户查询批改历史时，服务应当返回完整的推理轨迹以供审计
5. 当存储结果时，服务应当使用复合键（submission_id, question_id）以便高效检索
