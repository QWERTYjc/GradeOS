...
- [x] 1. 搭建项目结构和核心依赖
  ...
  - [x] 1.2 创建核心数据模型和类型定义
    ...
    - 定义枚举：FileType, SubmissionStatus, ReviewAction, QuestionType
    - 定义 ContextPack 和 EvidenceItem TypedDict
    - _需求：2.2, 3.2, 3.9, 7.1, 7.3_
...
- [ ] 3. 实现数据库层
  - [ ] 3.1 创建 PostgreSQL Schema 和迁移
    ...
    - grading_results 表增加字段：question_type, agent_type, evidence_chain, had_secondary_review
    - 为 agent_trace, student_feedback, evidence_chain 设置 JSONB 索引
    - _需求：7.1, 7.2, 7.3, 7.5, 11.6_
  - [ ] 3.2 实现仓储类
    - 创建 SubmissionRepository，包含 CRUD 操作
    - 创建 GradingResultRepository，处理复合键，支持 question_type 和 agent_type 查询
    - 创建 RubricRepository，支持 exam_id + question_id 查询
    - _需求：7.4, 9.1, 9.2, 9.3_

  - [x] 3.3 编写结果持久化完整性的属性测试
...
- [x] 8. 实现 LangGraph 批改智能体
  - [x] 8.1 定义 GradingState TypedDict
    - 包含所有字段：context_pack, vision_analysis, rubric_mapping, initial_score, reasoning_trace, critique_feedback, evidence_chain, final_score, confidence, agent_type, revision_count, is_finalized, needs_secondary_review
    - _需求：3.3, 3.9_
...
- [x] 20. 实现动态派生智能体架构
  ...
  - [x] 20.1 定义 QuestionType 枚举和 ContextPack 类型


    - 创建 QuestionType 枚举：OBJECTIVE, STEPWISE, ESSAY, LAB_DESIGN, UNKNOWN
    - 定义 ContextPack TypedDict：question_image, question_type, rubric, max_score, standard_answer, terminology, previous_result
    - 定义 EvidenceItem TypedDict：scoring_point, image_region, text_description, reasoning, rubric_reference, points_awarded

    - _需求：3.1, 3.2, 7.3_
  - [x] 20.2 实现 BaseGradingAgent 抽象基类

    - 定义 agent_type 属性
    - 定义 supported_question_types 属性
    - 定义 grade(context_pack) 抽象方法
    - _需求：11.5_

  - [x] 20.3 实现 AgentPool 智能体池管理

    - 实现 register_agent(agent) 方法
    - 实现 get_agent(question_type) 方法
    - 实现 list_agents() 方法
    - _需求：11.5_


  - [x] 20.4 实现 SupervisorAgent 总控调度智能体
    - 实现 analyze_question_type(image_data) 方法：调用 Gemini 分析题型
    - 实现 select_agent(question_type) 方法：从 AgentPool 选择智能体
    - 实现 build_context_pack() 方法：构建上下文包
    - 实现 spawn_and_grade(context_pack) 方法：派生智能体并执行批改
    - 实现 secondary_review() 方法：二次评估（置信度低时触发）
    - _需求：3.1, 3.2, 3.8_
  - [x] 20.5 编写 SupervisorAgent 题型分类一致性的属性测试
  ...
  - [x] 20.6 编写 Context Pack 结构完整性的属性测试
...
- [x] 21. 实现专业批改智能体
  ...
  - [x] 21.1 实现 ObjectiveAgent（选择题/判断题）


    - 继承 BaseGradingAgent
    - 实现 grade() 方法：依据标准答案进行精确比对
    - 生成证据链
    - _需求：11.1_

  - [x] 21.2 实现 StepwiseAgent（计算题）

    - 继承 BaseGradingAgent
    - 实现 grade() 方法：将解题过程拆解为步骤并逐步给分
    - 实现 extract_steps() 方法：提取解题步骤
    - 生成证据链
    - _需求：11.2_

  - [x] 21.3 实现 EssayAgent（作文/简答题）

    - 继承 BaseGradingAgent
    - 实现 grade() 方法：依据内容、结构、语言等维度评分
    - 生成证据链
    - _需求：11.3_

  - [x] 21.4 实现 LabDesignAgent（实验设计题）

    - 继承 BaseGradingAgent
    - 实现 grade() 方法：评估实验方案的完整性和科学性
    - 生成证据链
    - _需求：11.4_
  - [x] 21.5 编写智能体类型与题型匹配的属性测试
  ...
  - [x] 21.8 编写 StepwiseAgent 步骤分数一致性的属性测试
...
- [x] 10. 实现 Temporal Activities



  - [x] 10.1 创建 SegmentDocumentActivity

    - 封装 LayoutAnalysisService 调用
    - 返回包含题目区域的 SegmentationResult
    - _需求：2.1, 2.2_
  - [x] 10.2 创建 GradeQuestionActivity
    - 首先检查语义缓存
    - 缓存未命中时调用 SupervisorAgent 动态派生批改智能体
    - 低置信度时触发二次评估
    - 缓存高置信度结果
    - 返回包含 question_type, agent_type, evidence_chain 的 GradingResult
    - _需求：3.1, 3.8, 6.2, 6.3, 11.6_
  - [x] 10.3 创建 NotifyTeacherActivity

    - 当工作流进入 REVIEWING 状态时发送通知
    - _需求：5.2_
  - [x] 10.4 创建 PersistResultsActivity
...
- [x] 11. 实现 Temporal 工作流



  - [x] 11.1 创建 QuestionGradingChildWorkflow（子工作流）

    - 执行 GradeQuestionActivity，配置重试策略（最多 3 次）
    - 返回 GradingResult
    - _需求：4.4_
  - [x] 11.2 创建 ExamPaperWorkflow（父工作流）
...
- [x] 18. 创建 Docker 和 Kubernetes 配置





  - [x] 18.1 创建 Dockerfile


    - 为 API 服务创建 Dockerfile
    - 为 Temporal Workers 创建 Dockerfile
    - _需求：8.1_
  - [x] 18.2 创建 Kubernetes 清单
