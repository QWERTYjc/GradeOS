...
AI 批改系统是一套**完全摒弃 OCR** 的纯视觉批改引擎，直接利用多模态大模型（VLM）对试卷图像进行端到端语义理解。

| 能力 | 描述 | 状态 |
|------|------|------|
| 🎯 纯视觉批改 | 直接理解图像，无 OCR 误差传播 | ✅ |
| 🧠 动态派生智能体 | SupervisorAgent 根据题型调度专业智能体 | ✅ |
| ⚡ 持久化执行 | Temporal 工作流保证长周期任务可靠性 | ✅ |
| 👥 人机协作 | 低置信度结果人工审核介入 | ✅ |
| 📊 批量处理 | 多学生合卷自动识别与分割 | ✅ |
| 💰 成本优化 | Context Caching 节省约 25% Token | ✅ |

### 设计目标

- **日均处理能力**: 千万级请求
- **单题批改延迟**: < 30 秒（实测 15-20 秒）
- **评分准确度**: 与人工标注 Pearson 相关系数 > 0.9
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ Temporal 工作流  │ run_real_grading_workflow
│ 启动             │
└────────┬────────┘
         │
         ▼
...
│ │     ↓      │ │
│ │ 智能体派生  │ │ ObjectiveAgent/StepwiseAgent/...
│ │     ↓      │ │
│ │ 多步推理    │ │ 视觉提取 → 评分映射 → 自我反思
│ │     ↓      │ │
│ │ 结果缓存    │ │
...
│   │   └── finalization.py
│   └── specialized/        # 专业智能体
│       ├── objective.py    # 选择题/判断题
│       ├── stepwise.py     # 计算题
│       ├── essay.py        # 作文/简答
│       └── lab_design.py   # 实验设计
│
├── services/               # 业务服务层 (21个服务)
...
│   ├── rate_limiter.py     # 限流器
│   ├── distributed_transaction.py  # 分布式事务
│   ├── tracing.py          # 追踪服务
│   ├── enhanced_api.py     # 增强API服务
│   ├── gemini_reasoning.py # Gemini推理
...
│   ├── enhanced_workflow.py  # 增强工作流混入 (1523行)
│   │   # EnhancedWorkflowMixin: 进度查询/事件接收/分布式锁
│   ├── batch_grading.py    # 批量批改工作流
│   ├── exam_paper.py       # 试卷处理工作流
│   └── question_grading.py # 题目批改工作流
│
...
- **置信度阈值**: 0.75 (低于此值触发二次评估)

**题型映射**：
| QuestionType | Agent | 用途 |
|--------------|-------|------|
| OBJECTIVE | ObjectiveAgent | 选择题/判断题 |
| STEPWISE | StepwiseAgent | 计算题 |
| ESSAY | EssayAgent | 作文/简答 |
...
│   │   └── console/        # 批改控制台
│   │       └── page.tsx    # 控制台主页 (WorkflowGraph + Results)
│   ├── components/         # UI 组件
│   │   ├── WorkflowGraph/  # ReactFlow 工作流可视化
│   │   ├── NodeInspector/  # 节点详情侧边栏
│   │   ├── ResultsView/    # 批改结果展示
...
| `/batch/grade-cached` | POST | 优化的批量批改 (Context Caching) |
