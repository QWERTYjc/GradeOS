# 快速参考卡片

## 系统概览

**AI 批改系统** - 生产级纯视觉自动评估引擎

### 核心特性
- 🎯 纯视觉优先（无 OCR）
- 🤖 智能体推理（LangGraph）
- 📊 多学生自动识别
- 🔄 持久化执行（Temporal）
- 👥 人工审核介入

## 快速开始

### 1. 安装依赖
```bash
uv sync
```

### 2. 配置环境
```bash
cp .env.example .env
# 编辑 .env，设置 GEMINI_API_KEY
```

### 3. 启动服务
```bash
# 启动 API
uvicorn src.api.main:app --reload

# 启动 Worker（另一个终端）
python -m src.workers.cognitive_worker
```

### 4. 测试批改
```bash
# 同步批改（推荐用于测试）
python test_full_grading.py

# 或使用 API
curl -X POST "http://localhost:8000/batch/grade-sync" \
  -F "rubric_file=@批改标准.pdf" \
  -F "answer_file=@学生作答.pdf" \
  -F "api_key=YOUR_API_KEY"
```

## API 端点速查

### 批量提交
| 端点 | 方法 | 说明 |
|------|------|------|
| `/batch/grade-sync` | POST | 同步批改 |
| `/batch/submit` | POST | 异步批改 |
| `/batch/status/{batch_id}` | GET | 查询状态 |
| `/batch/results/{batch_id}` | GET | 获取结果 |
| `/batch/ws/{batch_id}` | WS | 实时推送 |

### 提交管理
| 端点 | 方法 | 说明 |
|------|------|------|
| `/submissions` | POST | 创建提交 |
| `/submissions/{id}` | GET | 获取提交 |
| `/submissions/{id}/status` | GET | 查询状态 |

### 评分细则
| 端点 | 方法 | 说明 |
|------|------|------|
| `/rubrics` | POST | 创建细则 |
| `/rubrics/{id}` | GET | 获取细则 |
| `/rubrics/{id}` | PUT | 更新细则 |

### 人工审核
| 端点 | 方法 | 说明 |
|------|------|------|
| `/reviews` | GET | 获取待审核 |
| `/reviews/{id}` | POST | 提交审核 |

## 文件结构

```
src/
├── api/                    # API 层
│   ├── main.py            # 应用入口
│   ├── routes/            # 路由
│   │   ├── batch.py       # 批量提交
│   │   ├── submissions.py # 提交管理
│   │   ├── rubrics.py     # 评分细则
│   │   └── reviews.py     # 人工审核
│   └── middleware/        # 中间件
│
├── services/              # 业务逻辑
│   ├── student_identification.py  # 学生识别
│   ├── rubric_parser.py           # 标准解析
│   ├── strict_grading.py          # 严格批改
│   ├── layout_analysis.py         # 页面分割
│   └── cache.py                   # 缓存管理
│
├── agents/                # 智能体
│   ├── grading_agent.py   # 批改智能体
│   ├── supervisor.py      # 总控智能体
│   └── specialized/       # 专业智能体
│       ├── objective.py   # 选择题
│       ├── stepwise.py    # 计算题
│       ├── essay.py       # 作文题
│       └── lab_design.py  # 实验题
│
├── workflows/             # 工作流
│   ├── batch_grading.py   # 批量批改
│   ├── exam_paper.py      # 试卷级
│   └── question_grading.py # 题目级
│
├── models/                # 数据模型
│   ├── submission.py      # 提交模型
│   ├── grading.py         # 批改结果
│   ├── rubric.py          # 评分细则
│   └── state.py           # 状态定义
│
└── utils/                 # 工具函数
    ├── coordinates.py     # 坐标转换
    ├── hashing.py         # 哈希计算
    └── database.py        # 数据库工具
```

## 常用命令

### 开发
```bash
make dev              # 启动开发环境
make dev-logs         # 查看日志
make dev-stop         # 停止开发环境
```

### 测试
```bash
make test             # 运行所有测试
make test-unit        # 单元测试
make test-property    # 属性测试
make test-coverage    # 覆盖率报告
```

### 代码质量
```bash
make lint             # 代码检查
make format           # 代码格式化
make type-check       # 类型检查
make quality          # 所有检查
```

### 数据库
```bash
make db-migrate       # 运行迁移
make db-rollback      # 回滚迁移
make db-revision      # 创建迁移
```

### Kubernetes
```bash
make k8s-deploy       # 部署到 K8s
make k8s-status       # 查看状态
make k8s-logs-api     # API 日志
make k8s-logs-worker  # Worker 日志
```

## 关键概念

### 学生识别
系统采用两阶段策略：
1. **直接识别** - 从试卷上识别学生信息
2. **推理识别** - 通过题目顺序循环检测推断边界

### 评分标准
支持两种格式：
1. **标准格式** - 分离的答案键
2. **嵌入式格式** - 答案在题目页面上

### 批改流程
```
PDF 转图像 → 解析标准 → 识别学生 → 逐个批改 → 返回结果
```

### 智能体类型
- **ObjectiveAgent** - 选择题/判断题
- **StepwiseAgent** - 计算题
- **EssayAgent** - 作文题
- **LabDesignAgent** - 实验设计题
- **SupervisorAgent** - 动态派生

## 性能指标

| 指标 | 数值 |
|------|------|
| 页面分割 | 3-5 秒 |
| 单题批改 | 15-20 秒 |
| 2 学生完整 | 2-3 分钟 |
| 30 学生完整 | 30-45 分钟 |
| 单学生成本 | $0.20-0.25 |

## 环境变量

### 必需
```bash
GEMINI_API_KEY=your_api_key
DATABASE_URL=postgresql://user:pass@host:5432/db
REDIS_URL=redis://host:6379
```

### 可选
```bash
TEMPORAL_HOST=localhost:7233
S3_ENDPOINT=http://minio:9000
LOG_LEVEL=INFO
```

## 故障排查

### 问题：API 无法连接
```bash
# 检查 API 是否运行
curl http://localhost:8000/health

# 查看日志
docker-compose logs api
```

### 问题：Gemini API 错误
```bash
# 检查 API Key
echo $GEMINI_API_KEY

# 测试 API 连接
python -c "from langchain_google_genai import ChatGoogleGenerativeAI; ..."
```

### 问题：数据库连接失败
```bash
# 检查 PostgreSQL
psql $DATABASE_URL

# 运行迁移
alembic upgrade head
```

### 问题：Redis 连接失败
```bash
# 检查 Redis
redis-cli ping

# 查看 Redis 日志
docker-compose logs redis
```

## 文档导航

| 文档 | 说明 |
|------|------|
| `README.md` | 项目概览 |
| `QUICKSTART.md` | 快速开始 |
| `BATCH_API_GUIDE.md` | API 详细指南 |
| `PROJECT_STATUS.md` | 项目状态 |
| `TOKEN_CONSUMPTION_ANALYSIS.md` | 成本分析 |
| `GRADING_TEST_REPORT.md` | 测试报告 |
| `.kiro/specs/` | 需求规范 |

## 联系方式

- 📧 Email: support@example.com
- 💬 Issues: GitHub Issues
- 📚 Docs: https://docs.example.com

## 许可证

MIT

---

**最后更新**: 2025-12-13  
**版本**: 1.0.0

