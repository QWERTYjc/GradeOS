...
# 3. 配置密钥
# 编辑 k8s/secrets.yaml，替换所�?CHANGE_ME_IN_PRODUCTION
...
| 变量 | 说明 | 示例 |
|------|------|------|
| `LLM_API_KEY` | Gemini API 密钥 | `AIza...` |
| `DATABASE_URL` | PostgreSQL 连接字符�?| `postgresql://user:pass@host:5432/db` |
| `REDIS_URL` | Redis 连接字符�?| `redis://host:6379` |
...
| 变量 | 默认�?| 说明 |
|------|--------|------|
| `RATE_LIMIT_REQUESTS` | `100` | 每分钟请求限�?|
| `CACHE_TTL_DAYS` | `30` | 缓存过期时间（天�?|
| `WORKER_CONCURRENCY` | `10` | Worker 并发�?|
| `LOG_LEVEL` | `INFO` | 日志级别 |
...
系统使用 KEDA 根据 Temporal 队列深度自动扩缩容认�?Worker�?
...
- 认知 Worker：根�?Gemini API 配额调整
- 编排 Worker：通常 2-3 个副本足�?
- 配置合适的并发�?
