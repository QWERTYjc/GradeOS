...
def _generate_overall_assessment(percentage: float, weak_points: List[Dict[str, Any]]) -> str:
    ...
    if weak_points:
        weak_labels = [p.get("description", "")[:20] for p in weak_points[:3] if p.get("description")]
        if weak_labels:
...
def generate_class_summary(
    ...
    return {
        ...
        "common_weak_points": [
            {"description": wp[0], "count": wp[1]} 
            for wp in common_weak_points
        ],
