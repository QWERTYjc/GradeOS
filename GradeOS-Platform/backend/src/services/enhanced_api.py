...
class EnhancedAPIConfig:
    ...
    allowed_sort_fields: Set[str] = field(
        default_factory=lambda: {"created_at", "updated_at", "score", "status", "id"}
    )
    allowed_filter_fields: Set[str] = field(
        default_factory=lambda: {"status", "exam_id", "student_id", "question_id"}
    )
...
class EnhancedAPIService:
    ...
    async def handle_websocket(self, websocket: WebSocket, submission_id: str) -> None:
        """
            ...
            while self._running:
                try:
                    ...
                    message = await asyncio.wait_for(
                        websocket.receive_text(), timeout=self.config.websocket_heartbeat_interval
                    )
    ...
    async def broadcast_state_change(self, submission_id: str, state: Dict[str, Any]) -> int:
        """
    ...
    async def _get_submission_state(self, submission_id: str) -> Optional[Dict[str, Any]]:
        """获取提交的当前状态"""
        try:
            # 先尝试从 Redis 获取
            redis_client = self.pool_manager.get_redis_client()
            ...
            async with self.pool_manager.pg_connection() as conn:
                ...
                if row:
                    return {
                        ...
                        "max_total_score": (
                            float(row["max_total_score"]) if row["max_total_score"] else None
                        ),
    ...
    async def query_with_pagination(
        self, table: str, params: QueryParams, trace_id: Optional[str] = None
    ) -> PaginatedResponse:
        """
        分页查询
    ...
    async def query_with_field_selection(
        ...
        if not fields:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="必须指定至少一个字段"
            )
        ...
        try:
            async with self.pool_manager.pg_connection() as conn:
                ...
                if not row:
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND, detail=f"记录不存在: {record_id}"
                    )
    ...
    async def get_slow_queries(
        self, limit: int = 100, min_duration_ms: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        获取慢查询记录
    ...
    def create_rate_limit_response(
        retry_after: int, limit: int, window_seconds: int
    ) -> Dict[str, Any]:
        """
        创建限流响应
    ...
    def get_rate_limit_headers(
        retry_after: int, limit: int, remaining: int, reset_at: Optional[str] = None
    ) -> Dict[str, str]:
        """
        获取限流响应头
...
def create_enhanced_api_service(
    ...
    """
    config = EnhancedAPIConfig(slow_query_threshold_ms=slow_query_threshold_ms)
    return EnhancedAPIService(
        pool_manager=pool_manager, tracing_service=tracing_service, config=config
    )
