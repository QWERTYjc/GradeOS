...
class CacheWarmupService:
    ...
    async def warmup_on_startup(self) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"开始启动预热: threshold={self.high_confidence_threshold}, " f"days={self.warmup_days}"
        )
        ...
        try:
            ...
            async with self.pool_manager.pg_connection() as conn:
                result = await conn.execute(query, (self.high_confidence_threshold, cutoff_date))
    ...
    async def _warmup_batch(self, batch: List[Dict[str, Any]]) -> tuple[int, int]:
        """
        预热一批结果
    ...
    async def _write_to_redis_only(self, key: str, value: Any, ttl_seconds: int) -> None:
        """
    ...
    async def precompute_rubric_hash(
        self, rubric_id: str, rubric_text: str, exam_id: str, question_id: str
    ) -> Optional[str]:
        """
        预计算评分细则哈希
        ...
        try:
            ...
            # 缓存 7 天
            await self._write_to_redis_only(cache_key, cache_value, ttl_seconds=7 * 24 * 3600)

            logger.info(
                f"预计算评分细则哈希: rubric_id={rubric_id}, " f"hash={rubric_hash[:16]}..."
            )
    ...
    async def get_cached_rubric_hash(self, exam_id: str, question_id: str) -> Optional[str]:
        """
        获取缓存的评分细则哈希
        ...
        try:
            redis_client = self.pool_manager.get_redis_client()
            cache_key = (
                f"{self.cache_service.config.hot_cache_prefix}:rubric_hash:{exam_id}:{question_id}"
            )
    ...
    async def async_batch_warmup(self, submission_ids: List[str]) -> asyncio.Task:
        """
        异步批量预热
            ...
            try:
                ...
                async with self.pool_manager.pg_connection() as conn:
                    result = await conn.execute(
                        query, (submission_ids, self.high_confidence_threshold)
                    )
                    rows = await result.fetchall()
                ...
                logger.info(f"异步批量预热完成: cached={total_cached}, failed={total_failed}")
    ...
    async def invalidate_rubric_hash_cache(self, exam_id: str, question_id: str) -> bool:
        """
        使评分细则哈希缓存失效
