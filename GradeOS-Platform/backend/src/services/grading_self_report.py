...
class SelfReportIssue:
    ...
    """
    issue_id: str
    type: str  # low_confidence, missing_evidence, alternative_solution, memory_conflict, no_citation
...
def generate_self_report(
    ...
    for q in score_result.get("question_details", []):
        ...
        for spr in q.get("scoring_point_results", []):
            ...
            # 缺失证据
            if not evidence_text or evidence_text.strip() in ["", "无", "N/A", "null", "【原文引用】未找到"]:
                issues.append(SelfReportIssue(
                    issue_id=next_issue_id(),
    ...
    return {
        ...
        "memory_candidates": [
            ...
            for i in issues if i.should_create_memory
        ],
    }
