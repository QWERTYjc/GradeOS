...
class RateLimiter:
    ...
    async def acquire(self, key: str, max_requests: int, window_seconds: int) -> bool:
        ...
        try:
            ...
            logger.error(f"限流器 Redis 错误（默认允许）: {str(e)}, key={key}", exc_info=True)
            return True

        except Exception as e:
            # 其他未预期错误：记录日志，默认允许请求
            logger.error(f"限流器发生未预期错误（默认允许）: {str(e)}, key={key}", exc_info=True)
            return True
    ...
    async def get_remaining(self, key: str, max_requests: int, window_seconds: int) -> int:
        ...
        try:
            ...
            logger.error(f"查询剩余配额发生未预期错误: {str(e)}, key={key}", exc_info=True)
            return max_requests  # 保守估计
    ...
    async def reset(self, key: str, window_seconds: int) -> bool:
        ...
        try:
            ...
            logger.error(f"重置限流计数器发生未预期错误: {str(e)}, key={key}", exc_info=True)
            return False

    async def get_rate_limit_info(self, key: str, max_requests: int, window_seconds: int) -> dict:
        """
        获取限流详细信息
