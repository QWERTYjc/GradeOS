...
class StreamingService:
    ...
    async def _persist_event(self, stream_id: str, event: StreamEvent) -> None:
        """
        持久化事件到数据库
        ...
        try:
            async with self.db_pool.connection() as conn:
                ...
                logger.debug(
                    f"事件已持久化: stream_id={stream_id}, " f"sequence={event.sequence_number}"
                )
    ...
    async def get_events(
        ...
        try:
                ...
                if event_data is None:
                    ...
                    if self.enable_persistence:
                        ...
                        async for event in self._recover_events_from_db(
                            stream_id, current_index, current_index
                        ):
                            yield event
                            current_index = event.sequence_number + 1
                            recovered = True
                    ...
                    # 检查流是否已完成
                    last_seq = await self.redis_client.get(self._get_sequence_key(stream_id))

                    if last_seq is None:
                        # 流已过期或不存在，检查数据库
                        if self.enable_persistence:
                            has_more = await self._check_db_for_more_events(
                                stream_id, current_index
                            )
                            if not has_more:
            ...
            error_event = StreamEvent(
                ...
                data={"error": "redis_error", "message": str(e), "retry_suggestion": "请稍后重试"},
            )
            yield error_event
    ...
    async def _recover_events_from_db(
        self, stream_id: str, from_sequence: int, to_sequence: Optional[int] = None
    ) -> AsyncGenerator[StreamEvent, None]:
        """
        从数据库恢复事件
    ...
    async def _check_db_for_more_events(self, stream_id: str, from_sequence: int) -> bool:
        """
        检查数据库中是否还有更多事件
    ...
    def _generate_retry_suggestion(self, error_type: str, error_message: str) -> str:
        """
        根据错误类型生成重试建议
    ...
    async def get_stream_status(self, stream_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"stream_id": stream_id, "error": str(e), "exists": False}
