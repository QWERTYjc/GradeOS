...
from src.models.failure_pattern import FailurePattern, FailurePatternSummary, PatternType
...
class RuleMiner:
    ...
    async def analyze_overrides(self, override_logs: List[GradingLog]) -> List[FailurePattern]:
        """分析改判样本，识别高频失败模式
        ...
        # 提取阶段失败模式
        patterns.extend(self._identify_extraction_patterns(extraction_failures))

        # 规范化阶段失败模式
        patterns.extend(self._identify_normalization_patterns(normalization_failures))

        # 匹配阶段失败模式
        patterns.extend(self._identify_matching_patterns(matching_failures))

        # 评分阶段失败模式
        patterns.extend(self._identify_scoring_patterns(scoring_failures))
    ...
    def _is_normalization_failure(self, log: GradingLog) -> bool:
        ...
        if (
            ...
            and len(log.normalization_rules_applied) > 0
            and log.match_result is False
        ):
            return True
        # 规范化前后差异很大，但仍然匹配失败
        if (
            log.extracted_answer
            and log.normalized_answer
            and log.extracted_answer != log.normalized_answer
            and log.match_result is False
        ):
            return True
        return False
    ...
    def _identify_extraction_patterns(self, logs: List[GradingLog]) -> List[FailurePattern]:
        """识别提取阶段的失败模式"""
        if not logs:
            return []
    ...
    def _identify_normalization_patterns(self, logs: List[GradingLog]) -> List[FailurePattern]:
        """识别规范化阶段的失败模式"""
        if not logs:
            return []
    ...
    def _identify_matching_patterns(self, logs: List[GradingLog]) -> List[FailurePattern]:
        """识别匹配阶段的失败模式"""
        if not logs:
            return []
    ...
    def _identify_scoring_patterns(self, logs: List[GradingLog]) -> List[FailurePattern]:
        """识别评分阶段的失败模式"""
        if not logs:
            return []
        ...
        score_diffs = []
        for log in logs:
            if log.score is not None and log.override_score is not None:
    ...
    def generate_summary(
        self, override_logs: List[GradingLog], patterns: List[FailurePattern]
