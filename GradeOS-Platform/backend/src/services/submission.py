...
class SubmissionService:
    ...
    async def submit(self, request: SubmissionRequest) -> SubmissionResponse:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"文件验证失败: " f"submission_id={submission_id}, " f"error={str(e)}")
            ...
            if request.file_type == FileType.PDF:
                # PDF 转图像（300 DPI）
                logger.info(f"转换 PDF 为图像: " f"submission_id={submission_id}")
                try:
                    images_data = await convert_pdf_to_images(request.file_data, dpi=300)
                    logger.info(
                        f"PDF 转换完成: "
                        f"submission_id={submission_id}, "
                    ...
                    logger.error(
                        f"PDF 转换失败: " f"submission_id={submission_id}, " f"error={str(e)}"
                    )
                ...
                logger.info(f"使用图像文件: " f"submission_id={submission_id}")

            else:
                raise SubmissionServiceError(f"不支持的文件类型: {request.file_type}")

            # ===== 第三步：保存到对象存储 =====
            logger.info(
                f"保存文件到存储: " f"submission_id={submission_id}, " f"文件数={len(images_data)}"
            )

            try:
                file_paths = await self.storage.save_files(
                    images_data, submission_id, extension="jpg"
                )
                logger.info(
                    f"文件保存完成: " f"submission_id={submission_id}, " f"文件路径={file_paths}"
                )
            except Exception as e:
                logger.error(f"文件保存失败: " f"submission_id={submission_id}, " f"error={str(e)}")
                raise SubmissionServiceError(f"文件保存失败: {str(e)}") from e

            # ===== 第四步：创建数据库记录 =====
            logger.info(f"创建提交记录: " f"submission_id={submission_id}")
            ...
            try:
                ...
                logger.info(
                    f"提交记录创建完成: " f"submission_id={submission_data['submission_id']}"
                )
            except Exception as e:
                logger.error(
                    f"创建提交记录失败: " f"submission_id={submission_id}, " f"error={str(e)}"
                )
                # 清理已保存的文件
                try:
            ...
            if self.orchestrator:
                logger.info(f"启动工作流: " f"submission_id={submission_id}")
                ...
                try:
                    ...
                    logger.info(
                        f"工作流已启动: " f"submission_id={submission_id}, " f"run_id={run_id}"
                    )

                except Exception as e:
                    logger.error(
                        f"启动工作流失败: " f"submission_id={submission_id}, " f"error={str(e)}",
                        exc_info=True,
                    )
                    # 工作流启动失败不影响提交成功
                    # 可以通过后台任务重试
            else:
                logger.warning(f"编排器未配置，跳过工作流启动: " f"submission_id={submission_id}")
            ...
            logger.error(f"提交处理失败: " f"error={str(e)}", exc_info=True)
    ...
    async def get_status(self, submission_id: str) -> Optional[SubmissionStatusResponse]:
        ...
        try:
            ...
            logger.error(f"查询提交状态失败: " f"submission_id={submission_id}, " f"error={str(e)}")
