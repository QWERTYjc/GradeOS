...
class ReportBuilder:
    ...
    def _build_summary(
        ...
        high_severity_errors = sum(
            1 for err in errors if err.severity == Severity.HIGH
        )
    ...
    def _build_action_plan(
        ...
        immediate_actions = [
            ...
            if sugg.priority == Severity.HIGH
        ][:3]  # 最多 3 个
        ...
        short_term_goals = [
            sugg.description
            for sugg in suggestions
            if sugg.priority == Severity.MEDIUM
        ][:3]
        
        # 长期目标（学习建议）
        long_term_goals = [
            rec.description
            for rec in deep_analysis.learning_recommendations
        ][:3]
