...
def calculate_question_confidence(
    ...
    if total_weight == 0:
        # 所有得分点分值为0，使用简单平均
        return round(
            sum(p.point_confidence for p in point_results) / len(point_results),
            3
        )
...
def calculate_student_confidence(
    ...
    if total_max_score == 0:
        return round(
            sum(q.confidence for q in question_results) / len(question_results),
            3
        )
    
    weighted_sum = sum(
        q.confidence * q.max_score 
...
class ConfidenceReport:
    ...
    def needs_review(self) -> bool:
        ...
        return (
            ...
            len(self.low_confidence_points) > 0 or
            self.alternative_solution_count > 0
        )
