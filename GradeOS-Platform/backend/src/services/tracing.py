...
class TracingService:
    ...
    async def trace_span(
        ...
        try:
            ...
            await self.end_span(span, status=SpanStatus.ERROR, error_message=str(e))
            raise
    ...
    async def get_span(self, trace_id: str, span_id: str) -> Optional[TraceSpan]:
        """
        获取单个跨度
    ...
    async def list_slow_spans(
        self, threshold_ms: Optional[int] = None, limit: int = 100, since: Optional[datetime] = None
    ) -> List[TraceSpan]:
        """
        列出慢跨度
