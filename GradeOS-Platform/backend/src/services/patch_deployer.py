...
class PatchDeployer:
    ...
    async def deploy_canary(self, patch: RulePatch, traffic_percentage: float = 0.1) -> str:
        """灰度发布补丁
    ...
    async def monitor_deployment(self, deployment_id: str) -> Dict[str, Any]:
        ...
        """
        if deployment_id not in self._active_deployments:
            return {"status": "not_found", "message": f"部署 {deployment_id} 不存在"}
        ...
        try:
            ...
            logger.info(
                f"监控结果：{status} | " f"异常数 {len(anomalies)} | " f"行动 {action_taken}"
            )
    ...
    async def _update_patch_status(
        ...
        async with pool.acquire() as conn:
            ...
            """
            await conn.execute(
                query, status.value, deployment_scope, deployed_at, rolled_back_at, patch_id
            )

        logger.info(f"更新补丁状态：{patch_id} -> {status.value}")

    async def _write_deployment_config(
        self, deployment_id: str, patch: RulePatch, traffic_percentage: float, scope: str
    ) -> None:
        """写入部署配置
