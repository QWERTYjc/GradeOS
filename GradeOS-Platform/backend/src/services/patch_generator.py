...
class PatchGenerator:
    ...
    async def generate_patch(self, pattern: FailurePattern) -> Optional[RulePatch]:
        """根据失败模式生成候选补丁
    ...
    async def _generate_extraction_patch(self, pattern: FailurePattern) -> Optional[RulePatch]:
        """生成提取阶段的补丁
    ...
    async def _generate_normalization_patch(self, pattern: FailurePattern) -> Optional[RulePatch]:
        """生成规范化阶段的补丁
        ...
        content = {
            ...
            "enhancement": {
                ...
                "examples": [{"input": "100cm", "output": "1m", "rule": "unit_conversion"}],
            },
        }
    ...
    async def _generate_matching_patch(self, pattern: FailurePattern) -> Optional[RulePatch]:
        """生成匹配阶段的补丁
    ...
    async def _generate_scoring_patch(self, pattern: FailurePattern) -> Optional[RulePatch]:
        """生成评分阶段的补丁
    ...
    def _suggest_variants(self, pattern: FailurePattern) -> List[str]:
        ...
        # 根据模式特征建议变体
        # 这里返回一些通用的变体建议
        variants = ["大小写变体", "空格变体", "标点符号变体", "单位变体"]
    ...
    def _suggest_synonyms(self, pattern: FailurePattern) -> List[Dict[str, List[str]]]:
        ...
        # 根据模式特征建议同义词
        synonym_groups = [
            {"canonical": "正确", "synonyms": ["对", "是", "√", "✓", "yes"]},
            {"canonical": "错误", "synonyms": ["错", "否", "×", "✗", "no"]},
        ]
