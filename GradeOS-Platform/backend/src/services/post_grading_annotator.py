...
class PostGradingAnnotator:
    ...
    def generate_annotations_for_student(
        ...
        """
        submission_id = (
            student_result.get("student_key") or student_result.get("submission_id") or ""
        )
    ...
    def _generate_question_annotations(
        ...
        scoring_point_results = (
            question.get("scoring_point_results") or question.get("scoringPointResults") or []
        )

        # 确定是否有具体分数信息
        has_specific_score = score is not None and max_score is not None and max_score > 0

        # ========== 1. 步骤级别标注 ==========
        if steps and self.config.show_check_marks:
            for step in steps:
                step_annotations = self._generate_step_annotations(step, question_id, page_index)
    ...
    def _generate_step_annotations(
        ...
        if self.config.mode == AnnotationMode.MINIMAL:
            ...
            annotations.append(
                VisualAnnotation(
                    annotation_type=(
                        AnnotationType.SIMPLE_CHECK if is_correct else AnnotationType.SIMPLE_CROSS
                    ),
            ...
            annotations.append(
                VisualAnnotation(
                    annotation_type=(
                        AnnotationType.STEP_CHECK if is_correct else AnnotationType.STEP_CROSS
                    ),
                    bounding_box=mark_bbox,
                    text="",
                    color=self.config.correct_color if is_correct else self.config.wrong_color,
            ...
            if self.config.show_m_a_marks and mark_type in ["M", "A"]:
                ...
                annotations.append(
                    VisualAnnotation(
                        annotation_type=(
                            AnnotationType.M_MARK if mark_type == "M" else AnnotationType.A_MARK
                        ),
                        bounding_box=mark_bbox,
                        text=mark_text,
                        color=self.config.correct_color if is_correct else self.config.wrong_color,
