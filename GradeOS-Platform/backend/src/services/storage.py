...
class StorageService:
    ...
    async def save_file(
        self, file_data: bytes, submission_id: str, file_index: int, extension: str = "jpg"
    ) -> str:
        """
        保存文件到存储
    ...
    async def save_files(
        self, files_data: List[bytes], submission_id: str, extension: str = "jpg"
    ) -> List[str]:
        """
        批量保存文件
        ...
        for index, file_data in enumerate(files_data):
            file_path = await self.save_file(file_data, submission_id, index, extension)
