...
class MultiLayerCacheService:
    ...
    def __init__(self, pool_manager: UnifiedPoolManager, config: Optional[CacheConfig] = None):
        """
        初始化多层缓存服务
    ...
    def register_invalidation_callback(self, callback: Callable[[str], Awaitable[None]]) -> None:
        """
        注册缓存失效回调
    ...
    def unregister_invalidation_callback(self, callback: Callable[[str], Awaitable[None]]) -> None:
        """
        取消注册缓存失效回调
    ...
    async def _backfill_cache(self, key: str, value: Any, ttl_seconds: int) -> None:
        """
        回填缓存
    ...
    async def invalidate_with_notification(self, pattern: str) -> int:
        ...
        try:
            ...
            cursor = 0
            while True:
                cursor, keys = await redis_client.scan(cursor=cursor, match=full_pattern, count=100)
            ...
            # 发布失效通知
            await redis_client.publish(self.config.pubsub_channel, pattern)
    ...
    async def sync_workflow_state(self, workflow_id: str, state: dict) -> None:
        """
        同步工作流状态到 Redis
        ...
        try:
            ...
            # 添加时间戳
            state_with_timestamp = {**state, "_synced_at": datetime.now(timezone.utc).isoformat()}
            ...
            await redis_client.publish(channel, json.dumps(state_with_timestamp, default=str))
    ...
    async def subscribe_workflow_state(
        self, workflow_id: str, callback: Callable[[dict], Awaitable[None]]
    ...
    def get_stats_dict(self) -> dict:
        ...
        redis_hit_rate = self._stats.redis_hits / redis_requests if redis_requests > 0 else 0.0
