...
class RubricRegistry:
    ...
    def get_rubric_for_question(self, question_id: str) -> RubricQueryResult:
        """
        获取指定题目的评分标准
    ...
    def update_rubric(self, question_id: str, rubric: QuestionRubric) -> bool:
        """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                "rubrics": {qid: rubric.to_dict() for qid, rubric in self._rubrics.items()},
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "RubricRegistry":
        """从字典反序列化"""
        registry = cls(
            total_score=data.get("total_score", 100.0), version=data.get("version", "1.0")
        )
    ...
    def _extract_scoring_points(self, text: str, max_score: float) -> List[ScoringPoint]:
        """从文本中提取得分点"""
        scoring_points = []
        ...
        if matches:
            ...
            for i, score_str in enumerate(matches[1:], 1):
                ...
                scoring_points.append(
                    ScoringPoint(description=f"得分点{i}", score=score, is_required=True)
                )

        # 如果没有找到得分点，创建一个默认的
        if not scoring_points:
            scoring_points.append(
                ScoringPoint(description="完整解答", score=max_score, is_required=True)
            )
    ...
    def _create_default_rubric(self, question_id: str) -> QuestionRubric:
        ...
        return QuestionRubric(
            ...
            scoring_points=[
                ScoringPoint(
                    description="默认得分点", score=self._default_max_score, is_required=True
                )
            ],
            alternative_solutions=[],
