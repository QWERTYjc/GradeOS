...
"""
    ...
    def _truncate_by_priority(
        self, sections: Dict[PromptSection, str], max_tokens: int
        ...
        for section in self.PRIORITY_ORDER:
            ...
            if current_tokens + section_tokens <= max_tokens:
                ...
                min_tokens_threshold = (
                    50 if section in [PromptSection.SYSTEM, PromptSection.RUBRIC] else 100
                )
                ...
                for remaining_section in self.PRIORITY_ORDER:
                    if (
                        remaining_section in sections
                        and remaining_section not in result_sections
                        and remaining_section not in truncated_sections
                    ):
