...
class VisualAnnotation:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "VisualAnnotation":
        ...
        return cls(
            ...
            bounding_box=BoundingBox.from_dict(data.get("bounding_box") or data.get("boundingBox") or {}),
...
class PageAnnotations:
    ...
    def add_score_annotation(
        ...
        """添加分数批注"""
        color = AnnotationColor.GREEN.value if score >= max_score * 0.8 else (
            AnnotationColor.ORANGE.value if score >= max_score * 0.5 else AnnotationColor.RED.value
        )
        self.annotations.append(VisualAnnotation(
