...
class FailurePattern(BaseModel):
    ...
    pattern_id: str = Field(
        default_factory=lambda: f"pattern_{uuid4().hex[:8]}",
        description="失败模式唯一标识"
    )
    ...
    sample_log_ids: List[str] = Field(
        ...
        description="样本日志ID列表（用于追溯）"
    )
    confidence: float = Field(
        default=1.0,
        description="模式识别置信度（0.0-1.0）",
        ge=0.0,
        le=1.0
    )
    is_fixable: bool = Field(
        default=False,
        description="是否可通过规则修复"
    )
