...
class ToleranceRule(BaseModel):
    ...
    rule_type: str = Field(
        ..., description="规则类型：numeric（数值）、unit（单位）、synonym（同义词）"
    )
...
class CalibrationProfile(BaseModel):
    ...
    deduction_rules: Dict[str, float] = Field(
        default_factory=dict, description="扣分规则，如 {'spelling_error': 0.5, 'logic_error': 2.0}"
    )

    # 容差设置
    tolerance_rules: List[ToleranceRule] = Field(default_factory=list, description="容差规则列表")
    ...
    strictness_level: float = Field(
        default=0.5, ge=0.0, le=1.0, description="严格程度，0.0 最宽松，1.0 最严格"
    )
