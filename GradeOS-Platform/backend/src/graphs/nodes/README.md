...
本模块包含将 Temporal Activity 重写�?LangGraph Node 的实现�?

## 概述

在从 Temporal 迁移�?LangGraph 的过程中，我们将原有�?Activity 重写�?Node 函数。每�?Node 都是一个纯函数，接�?`GradingGraphState` 作为输入，返回更新后的状态�?
...
**功能**: 使用 LayoutAnalysisService 调用 LLM 2.5 Flash Lite 识别试卷中的题目边界�?
...
**功能**: 首先检查语义缓存，缓存未命中时调用 LangGraph 智能体进行批改。对于高置信度结果（> 0.9），将其缓存�?Redis�?
...
- `percentage`: 20.0 + (完成题目�?/ 总题目数 * 60.0)
...
**输出状态字�?*:
- `progress.persistence_completed`: 持久化完成标�?
- `current_stage`: "persistence_completed"
- `percentage`: 90.0

**数据库操�?*:
1. 保存各题目的批改结果�?`grading_results` �?
2. 更新提交的总分
3. 更新提交状态为 `COMPLETED`
...
**输出状态字�?*:
- `progress.notification_sent`: 通知发送标�?
...
1. **可重试错�?*: 临时故障（如 API 限流、网络超时）会自动重�?
2. **不可重试错误**: 参数错误、逻辑错误不进行重�?
3. **降级处理**: 重试耗尽时执行降级逻辑，记录错误并标记需要人工介�?
4. **部分失败**: 某些节点（如通知）失败不会中断整个流�?
...
## �?Temporal Activity 的对�?

| 特�?| Temporal Activity | LangGraph Node |
|-----|------------------|----------------|
| 定义方式 | `@activity.defn` 装饰�?| 纯函�?|
| 状态管�?| 隐式（Temporal 管理�?| 显式（State 对象�?|
...
这些节点将在后续任务中组装成完整�?Graph�?

- `ExamPaperGraph`: 试卷批改 Graph
- `BatchGradingGraph`: 批量批改 Graph
- `RuleUpgradeGraph`: 规则升级 Graph

参见 `.kiro/specs/temporal-to-langgraph-migration/tasks.md` 中的任务 5 �?6�?
