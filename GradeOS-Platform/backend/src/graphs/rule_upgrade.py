...
async def mine_rules_node(state: RuleUpgradeGraphState) -> RuleUpgradeGraphState:
    ...
    logger.info(f"开始规则挖掘: upgrade_id={upgrade_id}, " f"time_window={time_window}")
    ...
    try:
        ...
        mined_rules = await miner.mine(
            start_time=time_window.get("start"), end_time=time_window.get("end")
        )

        # 筛选候选规则（置信度 > 0.8）
        rule_candidates = [rule for rule in mined_rules if rule.get("confidence", 0) > 0.8]
        ...
        return {
            **state,
            ...
            "percentage": 20.0,
            "timestamps": {**state.get("timestamps", {}), "mining_at": datetime.now().isoformat()},
        }

    except Exception as e:
        logger.error(f"规则挖掘失败: upgrade_id={upgrade_id}, error={str(e)}", exc_info=True)
        ...
        return {**state, "errors": errors, "current_stage": "mining_failed"}
...
async def generate_patches_node(state: RuleUpgradeGraphState) -> RuleUpgradeGraphState:
    ...
    if not rule_candidates:
        ...
        return {**state, "generated_patches": [], "current_stage": "no_patches"}

    logger.info(f"开始补丁生成: upgrade_id={upgrade_id}, " f"候选规则数={len(rule_candidates)}")
    ...
    try:
        ...
        logger.info(f"补丁生成完成: upgrade_id={upgrade_id}, " f"生成 {len(patches)} 个补丁")
        ...
        logger.error(f"补丁生成失败: upgrade_id={upgrade_id}, error={str(e)}", exc_info=True)
        ...
        return {**state, "errors": errors, "current_stage": "patch_generation_failed"}
...
async def regression_test_node(state: RuleUpgradeGraphState) -> RuleUpgradeGraphState:
    ...
    logger.info(f"开始回归测试: upgrade_id={upgrade_id}, " f"补丁数={len(patches)}")
    ...
    try:
        ...
        # 检查是否有回归
        regression_detected = any(result.get("regression", False) for result in test_results)

        logger.info(
            f"回归测试完成: upgrade_id={upgrade_id}, " f"regression_detected={regression_detected}"
        )
        ...
        logger.error(f"回归测试失败: upgrade_id={upgrade_id}, error={str(e)}", exc_info=True)
        ...
        return {**state, "errors": errors, "current_stage": "regression_test_failed"}
...
async def approval_interrupt_node(state: RuleUpgradeGraphState) -> RuleUpgradeGraphState:
    ...
    logger.info(
        f"收到审批响应: upgrade_id={upgrade_id}, " f"approved={approval_response.get('approved')}"
    )

    return {
        **state,
        "approval_result": approval_response,
        "timestamps": {**state.get("timestamps", {}), "approval_at": datetime.now().isoformat()},
    }
...
async def deploy_node(state: RuleUpgradeGraphState) -> RuleUpgradeGraphState:
    ...
    try:
        ...
        logger.info(f"部署完成: upgrade_id={upgrade_id}, " f"version={deployed_version}")

        return {
            **state,
            "deployment_status": "deployed",
            "deployed_version": deployed_version,
            "progress": {**state.get("progress", {}), "deployment_completed": True},
        ...
        logger.error(f"部署失败: upgrade_id={upgrade_id}, error={str(e)}", exc_info=True)
...
async def monitor_node(state: RuleUpgradeGraphState) -> RuleUpgradeGraphState:
    ...
    logger.info(f"开始监控: upgrade_id={upgrade_id}, " f"version={deployed_version}")
    ...
    return {
        **state,
        "progress": {**state.get("progress", {}), "monitoring_completed": True},
        "current_stage": "completed",
        "percentage": 100.0,
        "timestamps": {**state.get("timestamps", {}), "completed_at": datetime.now().isoformat()},
    }
...
async def rollback_node(state: RuleUpgradeGraphState) -> RuleUpgradeGraphState:
    ...
    logger.info(f"开始回滚: upgrade_id={upgrade_id}, " f"version={deployed_version}")
    ...
    try:
        ...
        logger.error(f"回滚失败: upgrade_id={upgrade_id}, error={str(e)}", exc_info=True)
        ...
        return {**state, "errors": errors, "current_stage": "rollback_failed"}
...
def should_continue_after_approval(state: RuleUpgradeGraphState) -> Literal["deploy", "end"]:
    """判断审批后是否继续"""
...
def create_rule_upgrade_graph(
    checkpointer: Optional[AsyncPostgresSaver] = None, require_approval: bool = True
) -> StateGraph:
    """创建规则升级 Graph
    ...
    graph.add_conditional_edges(
        "mine_rules", should_generate_patches, {"generate": "generate_patches", "end": END}
    )
    ...
    if require_approval:
        graph.add_conditional_edges(
            "regression_test", should_deploy, {"deploy": "approval_interrupt", "end": END}
        )

        # 审批后的条件路由
        graph.add_conditional_edges(
            "approval_interrupt", should_continue_after_approval, {"deploy": "deploy", "end": END}
        )
    else:
        graph.add_conditional_edges(
            "regression_test", should_deploy, {"deploy": "deploy", "end": END}
        )
...
def create_scheduled_rule_upgrade_graph(
    ...
    Returns:
        编译后的 Graph
    """
    return create_rule_upgrade_graph(checkpointer=checkpointer, require_approval=False)
