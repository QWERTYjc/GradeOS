...
async def understand_assignment_node(state: AssistantGradingState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[AssistantGrading] 理解分析完成: analysis_id={analysis_id}, knowledge_points={len(understanding['knowledge_points'])}"
        )
        ...
        return {
            ...
            + [{"stage": "understand", "error": str(e), "timestamp": datetime.now().isoformat()}],
...
async def identify_errors_node(state: AssistantGradingState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[AssistantGrading] 错误识别完成: analysis_id={analysis_id}, 发现 {len(errors)} 个错误"
        )
...
async def generate_suggestions_node(state: AssistantGradingState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[AssistantGrading] 建议生成完成: analysis_id={analysis_id}, 生成 {len(suggestions)} 条建议"
        )
...
async def deep_analysis_node(state: AssistantGradingState) -> Dict[str, Any]:
    ...
    try:
        ...
        if knowledge_points:
            avg_confidence = sum(kp.get("confidence", 0.5) for kp in knowledge_points) / len(
                knowledge_points
            )
        ...
        # 下一步计划
        next_steps = ["针对错误进行改正", "加强薄弱知识点的学习", "完成类似题目的练习"]
        ...
        logger.info(
            f"[AssistantGrading] 深度分析完成: analysis_id={analysis_id}, overall_score={overall_score:.1f}"
        )
        ...
        return {
            "processing_errors": state.get("processing_errors", [])
            + [
                {"stage": "deep_analysis", "error": str(e), "timestamp": datetime.now().isoformat()}
            ],
            "retry_count": state.get("retry_count", 0) + 1,
        }
...
async def generate_report_node(state: AssistantGradingState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[AssistantGrading] 报告生成完成: analysis_id={analysis_id}, overall_score={report['summary']['overall_score']:.1f}"
        )
