...
class BatchGradingGraphState(TypedDict, total=False):
    ...
    batch_progress: Annotated[
        Dict[str, Any], last_value
    ]  # 批次进度信息（使用 last_value reducer 处理并发）
    batch_retry_needed: Annotated[
        Dict[str, Any], last_value
    ]  # 需要重试的批次信息（使用 last_value reducer 处理并发）
...
def create_initial_batch_state(
    batch_id: str, exam_id: str, pdf_path: str, rubric: str
...
def create_initial_upgrade_state(
    upgrade_id: str, trigger_type: str, time_window: Dict[str, str]
