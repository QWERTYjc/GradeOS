...
async def analyze_assignment(
    ...
    try:
        ...
        logger.info(f"[AssistantAPI] 收到分析请求: analysis_id={analysis_id}, images={len(request.images)}")
...
async def batch_analyze_assignments(
    ...
    try:
        ...
        logger.info(f"[AssistantAPI] 收到批量分析请求: batch_id={batch_id}, count={len(request.analyses)}")
        ...
        logger.info(f"[AssistantAPI] 批量分析任务已启动: batch_id={batch_id}, count={len(analysis_ids)}")
...
async def _run_analysis_workflow(
    ...
    try:
        ...
        graph = create_assistant_grading_graph(
            checkpointer=orchestrator.checkpointer if hasattr(orchestrator, 'checkpointer') else None
        )
        ...
        async for state_update in graph.astream(initial_state, config=config):
            ...
            # 广播进度更新
            await _broadcast_message(analysis_id, {
                "type": "progress",
                "analysis_id": analysis_id,
