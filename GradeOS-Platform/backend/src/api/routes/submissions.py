    ...
    **需求：1.3**
    ...
    try:
        ...
        request = SubmissionRequest(
            exam_id=exam_id, student_id=student_id, file_type=file_type, file_data=file_data
        )
        ...
        submission_service = SubmissionService(
            repository=submission_repo, storage=storage_service, orchestrator=orchestrator
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"提交处理失败: {str(e)}"
        )
...
async def get_submission_status(submission_id: str) -> SubmissionStatusResponse:
    ...
    **需求：7.4**
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"查询提交状态失败: {str(e)}"
        )
...
async def get_grading_results(submission_id: str) -> ExamPaperResult:
    ...
    **需求：7.4**
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"查询批改结果失败: {str(e)}"
        )
