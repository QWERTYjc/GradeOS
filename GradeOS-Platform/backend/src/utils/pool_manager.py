    ...
    def from_env(cls) -> "PoolConfig":
        ...
        if not pg_dsn:
            ...
            if pg_host:
                ...
                pg_dsn = (
                    f"postgresql://{pg_user}:{pg_password}@" f"{pg_host}:{pg_port}/{pg_database}"
                )
...
class UnifiedPoolManager:
    ...
    async def _init_pg_pool(self) -> None:
        ...
        self._pg_pool = AsyncConnectionPool(
            ...
            # 连接重连配置
            reconnect_timeout=(
                self._config.pg_reconnect_timeout
                if hasattr(self._config, "pg_reconnect_timeout")
                else 5.0
            ),
        )
        await asyncio.wait_for(self._pg_pool.open(), timeout=10.0)
        logger.info(
            f"PostgreSQL 连接池初始化完成: min={self._config.pg_min_size}, max={self._config.pg_max_size}, idle_timeout={self._config.pg_idle_timeout}s"
        )
    ...
    async def shutdown(self, timeout: Optional[float] = None) -> None:
        ...
        while self._active_operations > 0:
            ...
            if elapsed >= timeout:
                logger.warning(f"关闭超时，仍有 {self._active_operations} 个操作进行中，强制关闭")
                break
    ...
    async def health_check(self) -> dict:
        ...
        if self._pg_pool is not None:
            try:
                ...
                result["postgresql"] = {"status": "unhealthy", "details": {"error": str(e)}}
        else:
            result["postgresql"] = {"status": "not_configured", "details": {}}
        ...
        if self._redis_client is not None:
            try:
                ...
                result["healthy"] = False
                result["redis"] = {"status": "unhealthy", "details": {"error": str(e)}}
        else:
            result["redis"] = {"status": "not_configured", "details": {}}
...
async def init_pool_manager(config: Optional[PoolConfig] = None, **kwargs) -> UnifiedPoolManager:
    """
    初始化并获取统一连接池管理器
