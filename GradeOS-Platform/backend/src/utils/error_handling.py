    ...
    **kwargs
    ...
    for retry_count in range(config.max_retries + 1):
        try:
            ...
            if retry_count > 0:
                logger.info(
                    f"函数 {func.__name__} 在第 {retry_count} 次重试后成功执行"
                )
...
class PartialResults:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completion_rate": self.completed_count / self.total_items if self.total_items > 0 else 0,
...
async def execute_with_partial_save(
    ...
    try:
        ...
        if partial_results.failed_count > 0:
            logger.warning(
                f"批次 {batch_id} 有 {partial_results.failed_count} 个失败项，"
                f"保存部分结果..."
            )
