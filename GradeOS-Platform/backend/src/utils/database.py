...
class Database:
    ...
    def is_available(self) -> bool:
        """数据库是否可用"""
        return not self._degraded_mode and (self._use_unified_pool or self._pool is not None)
    ...
    async def connection(self):
        ...
        if self._degraded_mode:
            raise RuntimeError("数据库不可用（降级模式）。" "请使用内存缓存或其他替代方案。")
...
async def get_db_pool() -> AsyncConnectionPool:
    ...
    if db._use_unified_pool:
        logger.warning(
            "使用统一连接池时，不应直接访问连接池，请使用 db.connection() 或 db.transaction()"
        )
        return None
