...
class SubmissionRepository:
    ...
    async def update_status(self, submission_id: str, status: str | SubmissionStatus) -> bool:
        """更新提交状态"""
        # 如果是枚举类型，转换为字符串
        ...
        async with self.db.connection() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    query, {"submission_id": UUID(submission_id), "status": status_value}
                )
                return cur.rowcount > 0

    async def update_scores(
        self, submission_id: str, total_score: float, max_total_score: float
    ) -> bool:
        """更新总分"""
        query = """
            UPDATE submissions
    ...
    async def get_by_student(
        self, student_id: str, limit: int = 10, offset: int = 0
    ) -> List[Dict[str, Any]]:
        """获取学生的提交记录列表"""
        query = """
            SELECT submission_id, exam_id, student_id, status, total_score,
        ...
        async with self.db.connection() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    query, {"student_id": UUID(student_id), "limit": limit, "offset": offset}
                )
                results = await cur.fetchall()
                return [self._format_submission(row) for row in results]

    async def get_by_exam(
        self, exam_id: str, limit: int = 100, offset: int = 0
    ) -> List[Dict[str, Any]]:
        """获取考试的提交记录列表"""
        query = """
            SELECT submission_id, exam_id, student_id, status, total_score,
        ...
        async with self.db.connection() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    query, {"exam_id": UUID(exam_id), "limit": limit, "offset": offset}
                )
                results = await cur.fetchall()
                return [self._format_submission(row) for row in results]
