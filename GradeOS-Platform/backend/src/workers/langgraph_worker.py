...
class LangGraphWorker:
    ...
    def __init__(self, poll_interval: float = 1.0, max_concurrent_runs: int = 10):
        """初始化 Worker
    ...
    def _register_graphs(self):
        ...
        self.orchestrator.register_graph("exam_paper", create_exam_paper_graph())
        self.orchestrator.register_graph("batch_grading", create_batch_grading_graph())
        self.orchestrator.register_graph("rule_upgrade", create_rule_upgrade_graph())
    ...
    async def _recover_interrupted_runs(self):
        ...
        try:
            ...
            running_runs = await self.orchestrator.list_runs(status=RunStatus.RUNNING, limit=100)
    ...
    async def _shutdown(self):
        ...
        if self._current_runs:
            ...
            try:
                await asyncio.wait_for(self._wait_for_runs(), timeout=30.0)
            ...
            try:
                ...
                if available_slots > 0:
                    ...
                    for run in pending_runs:
                        # 启动执行
                        asyncio.create_task(self._execute_run(run["run_id"], run["graph_name"]))
    ...
    async def _execute_run(self, run_id: str, graph_name: str):
        ...
        try:
            ...
            row = await self.orchestrator.db_pool.fetchrow(
                "SELECT input_data FROM runs WHERE run_id = $1", run_id
            )
            ...
            await self.orchestrator._run_graph_background(
                run_id=run_id, graph_name=graph_name, compiled_graph=compiled_graph, payload=payload
            )
            ...
            # 更新状态为 failed
            await self.orchestrator._update_run_status(run_id, "failed", error=str(e))
        finally:
...
async def main():
    ...
    worker = LangGraphWorker(poll_interval=poll_interval, max_concurrent_runs=max_concurrent)
