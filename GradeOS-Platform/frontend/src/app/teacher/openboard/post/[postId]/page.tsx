...
export default function TeacherPostDetailPage() {
  ...
  const handleDeletePost = async () => {
    ...
    try {
      await openboardApi.deletePost(post.post_id, user.id, '管理员删除');
      router.push(`/teacher/openboard/${post.forum_id}`);
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={() => router.push(`/teacher/openboard/${post.forum_id}`)}
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-slate-400">共 {replies.length + 1} 层</span>
            <button
              onClick={handleDeletePost}
              className="px-4 py-2 text-red-500 hover:bg-red-50 rounded-lg text-sm transition-colors cursor-pointer"
            >
              删除帖子
            </button>
          </div>
