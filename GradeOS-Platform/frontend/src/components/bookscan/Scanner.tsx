...
import { Camera, Upload, X, Loader2, Zap, ZapOff, Sparkles, BookOpen, Smartphone, ScanLine, Info, CheckCircle2, FileUp } from 'lucide-react';
...
export default function Scanner() {
  ...
  const processPage = async (source: HTMLCanvasElement, sx: number, sy: number, sw: number, sh: number, tag: string) => {
    ...
    await handleImageData(dataUrl, tag);
  };
  ...
  const captureFrame = useCallback(async () => {
    ...
    if (scanMode === 'single') {
      await processPage(canvas, 0, 0, canvas.width, canvas.height, 'single');
    } else {
      const halfW = canvas.width / 2;
      await processPage(canvas, 0, 0, halfW, canvas.height, 'left');
      await processPage(canvas, halfW, 0, halfW, canvas.height, 'right');
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || !context?.currentSessionId) return;

    setIsProcessing(true);
    setFeedbackMessage(`Importing ${files.length} images...`);
    ...
    try {
      ...
      setFeedbackMessage("Import complete");
    } catch (error) {
    ...
    <div className="flex flex-col h-full bg-white text-[#0B0F17]">
      ...
      <div className="relative flex-1 bg-[#F5F7FB] flex items-center justify-center overflow-hidden">
        ...
        {(!isCameraActive || isProcessing) && (
          <div className="text-center p-12 max-w-sm animate-in fade-in zoom-in duration-500 relative z-30">
            {isProcessing ? (
               <div className="flex flex-col items-center">
                  ...
                  <h2 className="text-2xl font-black mb-2 tracking-tight">AI Processing</h2>
                  <p className="text-slate-500 text-sm font-medium">Applying industrial enhancements via TextIn Engine...</p>
               </div>
            ) : (
              <>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="w-full py-5 bg-white border-2 border-slate-100 text-slate-600 hover:border-blue-100 hover:text-blue-600 rounded-3xl font-bold transition-all flex items-center justify-center gap-3">
                    <FileUp size={20} />
                    Import Local Assets
                  </button>
                </div>
      ...
      <div className="bg-white border-t border-slate-100 px-8 py-8 pb-14 safe-area-bottom shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
        <div className="max-w-xl mx-auto flex flex-col gap-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <button 
              onClick={isCameraActive ? stopCamera : () => fileInputRef.current?.click()} 
              className="flex flex-col items-center gap-2 text-slate-400 group"
            >
               <div className="w-16 h-16 rounded-[2rem] flex items-center justify-center border-2 border-slate-50 bg-slate-50 group-hover:bg-blue-50 group-hover:text-blue-600 transition-all">
                 {isCameraActive ? <X size={28} /> : <Upload size={28} />}
               </div>
               <span className="text-[10px] font-black text-slate-300 tracking-widest uppercase">
                 {isCameraActive ? 'EXIT' : 'UPLOAD'}
               </span>
            </button>
          </div>
        </div>
