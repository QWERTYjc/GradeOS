...
GradeOS 是一个基�?**Google Gemini 3.0 Flash** 大模型的智能作业批改系统，支持：
...
1. **上传文件**：上传评分标�?PDF 和学生作�?PDF/图片
2. **AI 解析**：系统自动解析评分标准，识别学生边界
3. **评分标准确认**：教师审�?AI 解析的评分细�?
4. **智能批改**：AI 根据评分标准逐题批改
5. **结果审核**：教师审核批改结果，可修正分�?
6. **成绩导出**：将成绩导入班级系统
...
| GET | `/api/batch/status/{batch_id}` | 查询批改状�?|
| GET | `/api/batch/rubric/{batch_id}` | 获取解析的评分标�?|
...
├── graphs/
�?  └── batch_grading.py     # LangGraph 批改工作�?
├── services/
�?  ├── llm_reasoning.py  # Gemini API 调用
�?  ├── rubric_parser.py     # 评分标准解析
�?  ├── student_identification.py # 学生识别
�?  └── strict_grading.py    # 严格批改逻辑
└── orchestration/
    └── langgraph_orchestrator.py # 工作流编�?
```
...
| 变量�?| 描述 | 默认�?|
|--------|------|--------|
| `LLM_API_KEY` | Google Gemini API 密钥 | 必填 |
| `DATABASE_URL` | PostgreSQL 连接�?| 使用 SQLite |
| `OFFLINE_MODE` | 离线模式（跳�?DB�?| false |
...
1. �?`services/` 中添加业务逻辑
2. �?`graphs/batch_grading.py` 中添加节�?
3. �?`routes/` 中添�?API 端点
4. 前端对应更新组件和状�?
